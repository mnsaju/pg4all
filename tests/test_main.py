...
def test_preview_endpoint_returns_findings_for_an_overcommitted_set(client):
    resp = client.post(
        "/preview",
        data={
...
def test_preview_endpoint_is_quiet_for_a_sane_set(client):
    resp = client.post(
        "/preview",
        data={
...
# --- seeing and downloading the conf -----------------------------------

def test_the_tuner_shows_the_conf_it_would_generate(client):
    """For a tool whose whole output is a postgresql.conf, never showing
    it was a strange gap."""
    resp = client.get("/")
    assert resp.status_code == 200
    assert "Generated postgresql.conf" in resp.text
    assert "# Generated by pg4all" in resp.text
    assert "shared_buffers" in resp.text


def test_the_preview_reflects_the_submitted_values(client):
    resp = client.post(
        "/preview",
        data={
            "pg_version": "17", "workload": "oltp", "tier": "medium",
            "p_max_connections": "250",
        },
    )
    assert resp.status_code == 200
    assert "max_connections = 250" in resp.text


def test_the_preview_is_the_same_text_the_build_writes(client):
    """The preview must not be a second rendering that can drift from what
    is actually baked into the image."""
    fields = {
        "pg_version": "17", "workload": "oltp", "tier": "medium",
        "p_work_mem": "12", "extensions": ["pg_stat_statements"],
    }
    previewed = client.post("/preview.conf", data=fields).text

    built = _submit_build(client, p_work_mem="12", extensions=["pg_stat_statements"])
    build_id = _BUILD_ID_RE.search(built.text).group(1)
    on_disk = (
        main_module.BUILD_OUTPUT_DIR / build_id / "postgresql.conf"
    ).read_text()

    assert previewed == on_disk


def test_the_conf_downloads_as_a_file(client):
    resp = client.post(
        "/preview.conf",
        data={"pg_version": "17", "workload": "oltp", "tier": "medium"},
    )
    ...
    assert resp.headers["content-disposition"] == (
        'attachment; filename="postgresql.conf"'
    )
    assert resp.text.startswith("# Generated by pg4all")
    assert "listen_addresses" in resp.text


def test_a_builds_conf_can_be_downloaded_afterwards(client):
    resp = _submit_build(client, p_max_connections="180")
    build_id = _BUILD_ID_RE.search(resp.text).group(1)

    downloaded = client.get(f"/builds/{build_id}/postgresql.conf")
    assert downloaded.status_code == 200
    assert "max_connections = 180" in downloaded.text
    assert build_id[:8] in downloaded.headers["content-disposition"]


def test_downloading_the_conf_of_an_unknown_build_is_404(client):
    assert client.get(f"/builds/{'f' * 32}/postgresql.conf").status_code == 404
    assert client.get("/builds/not-a-build-id/postgresql.conf").status_code == 404


def test_the_conf_download_needs_a_session(anonymous_client):
    for method, path in (
        ("POST", "/preview.conf"),
        ("GET", f"/builds/{'a' * 32}/postgresql.conf"),
    ):
